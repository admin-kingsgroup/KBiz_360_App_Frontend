  ...
  return (
    <View className="mb-3">
      <View className="flex-row items-center gap-1 mb-1.5">
        <Text style={{ color: colors.coolText, fontSize: 12, fontWeight: '700', letterSpacing: 0.3 }}>{label}</Text>
        {required ? <Text style={{ color: colors.danger, fontSize: 12 }}>*</Text> : null}
      </View>
      {children}
      {hint ? <Text style={{ color: colors.coolText3, fontSize: 11, marginTop: 3 }}>{hint}</Text> : null}
    </View>
  );
}
