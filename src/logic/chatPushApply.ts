...
export interface ApplyResult {
  ...
  changed: boolean;
  /** Total unread across all conversations AFTER applying — feeds the app-icon badge. */
  totalUnread: number;
}

const sumUnread = (cs: ChatConversation[]): number => cs.reduce((s, c) => s + (c.unread || 0), 0);

export function applyChatPush(conversations: ChatConversation[], data: ChatPushData): ApplyResult {
  const convId = data.conversationId;
  if (!convId) return { conversations, changed: false, totalUnread: sumUnread(conversations) };
  ...
  if (existing && data.messageId && existing.lastMessage?.messageId === data.messageId) {
    return { conversations, changed: false, totalUnread: sumUnread(conversations) };
  }
  ...
  return { conversations: sorted, changed: true, totalUnread: sumUnread(sorted) };
}
